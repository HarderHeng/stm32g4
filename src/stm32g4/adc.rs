//! STM32G4 ADC implementation for R3_2 (3-shunt dual-ADC) current sensing
//!
//! Concrete implementation of the CurrentSense trait for STM32G4 ADC1/ADC2.
...
use crate::hal::adc::{CurrentSense, CalibrateError};
...
const COUNTS_PER_AMP: f32 = ADC_FULL_SCALE * R_SHUNT * OPAMP_GAIN / VDDA;
...
pub struct Stm32G4Adc {
    ...
    ib_offset: u16,
    ic_offset: u16,
    /// Last injected sample, updated in `update_sector` / read by `read_currents`.
    raw_adc1: u16,
    raw_adc2: u16,
...
impl CurrentSense for Stm32G4Adc {
    #[inline(always)]
    ...
    fn update_sector(&mut self, sector: u8) {
        ...
        //    changing self.sector so that read_currents decodes correctly.
        self.latch_injected();
        ...
        for _ in 0..N {
            ...
            while !pac::ADC1.isr().read().jeos() {
                ...
                if timeout > TIMEOUT_LIMIT {
                    return Err(CalibrateError::Timeout);
                }
            }
